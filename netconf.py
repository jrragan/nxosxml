...
from xmlFunctions import buildclienthello, rpcparse, buildnxosmessage, buildnxosclimessage
    ...
    def decorator(self, *args, **kwargs):
        if self.ncconnected:
            ...
            try:
                ...
                self.logger.error("netconfbuilder: Error building the rpc command ")
            ...
            try:
                ...
                logger.debug("netconfbuilder: Error sending xml message")
                raise
        else:
...
class NxosConnect(SshConnect):
    ...
    def get_xmlxserverxtatus(self):
        ...
        """
        if self.ncconnected:
            return self.nxoscli(['show xml server status'])
        else:
            return False
...
if __name__ == "__main__":
    ...
    nxostest = NxosConnect(host="192.168.133.250")
    print nxostest.sshconnected
    print nxostest.ncconnected
    nxostest.nc_sshconnect(username="admin", password="cisco")
    print nxostest.sshconnected
    print nxostest.ncconnected
    nxos_snippet = """
      <show>
    ...
    print nxostest.nxosget(nxos_snippet, schema="vlan_mgr_cli", getfilter="subtree")
    print nxostest.nxoscli(['show vlan id 1000'])
    print nxostest.nxoscli(['show vlan'])
