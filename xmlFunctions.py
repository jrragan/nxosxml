...
def _rpc_error_parser(root):
    ...
    if error:
        ...
        raise NetConfRPCError("RPC Error from Server: {0}".format(etree.tostring(root)))
...
def buildclienthello(client_capabilities=('urn:ietf:params:xml:ns:netconf:base:1.0',)):
    ...
    client_hello = etree.tostring(helloelement, encoding="UTF-8", xml_declaration=True, pretty_print=True)
    return client_hello
...
def buildnxosmessage(nctype, schema, rpcmessageid, message=None, ncfilter=None):
    ...
    clientrpcelement = etree.tostring(rpcelement, encoding="UTF-8", xml_declaration=True, pretty_print=True)
...
def buildnxosclimessage(clilist, rpcmessageid):
    ...
    cli_message = etree.tostring(rpcelement, encoding="UTF-8", xml_declaration=True, pretty_print=True)
    return cli_message


def buildtoplevelelement(elementname, nsmapdict, rpcmessageid):
